...
test.beforeEach(async ({ page }) => {
  await page.route('**/api/**', async route => {
    ...
    if (/^\/api\/providers\/[^/]+\/versions$/.test(path)) return route.fulfill({ json: {providerId:decodeURIComponent(path.split('/')[3]),items:[{id:'v47',name:'4.7'},{id:'v48',name:'4.8'}]} })
...
test('control tower surfaces real task facts and configuration tools', async ({ page }) => {
  ...
  await page.getByLabel('工单反馈源').selectOption('Haru TAPD')
  await expect(page.getByRole('button', { name: /全部版本/ })).toHaveAttribute('aria-pressed', 'true')
  await page.getByRole('button', { name: /指定版本/ }).click()
  await page.getByLabel('4.7').check()
  await expect(page.getByLabel('4.7')).toBeChecked()
  await proof(page, 'project-version-filter-dark')
