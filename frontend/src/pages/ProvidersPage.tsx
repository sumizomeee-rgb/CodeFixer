...
export function ProvidersPage() {
  ...
  const load = async () => {
    ...
    catch (e) { setNotice(e instanceof Error ? e.message : '来源加载失败') }
  }
  useEffect(() => { void load() },[])
  ...
  const save = async () => {
    ...
    try {
      ...
      setSettings(next); setNotice(editingId ? '来源配置已更新' : '来源和登录信息已保存'); reset(); await load()
  ...
  return <section className="page-stack providers-page">
    <div className="page-heading"><div><h1>工单来源</h1><p>连接问题反馈平台；登录信息只保存在当前部署机的 .local 中。</p></div><button className="primary compact" onClick={openNew}>添加来源</button></div>
    {notice && <button className="notice-strip" onClick={() => setNotice('')}>{notice}</button>}
    {providers.length === 0 ? <div className="empty-state"><h2>还没有工单来源</h2><p>连接 Redmine 或 TAPD 后，CodeFixer 才能收取真实 Bug。</p><button className="primary compact" onClick={openNew}>连接来源</button></div> : <div className="provider-grid refined-provider-grid">{providers.map(provider => {
