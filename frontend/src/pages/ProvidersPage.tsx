...
export function ProvidersPage() {
  ...
  return <section className="page-stack providers-page">
    <div className="page-heading"><div><span className="page-kicker">INTAKE CHANNELS</span><h1>工单反馈来源</h1><p>连接日常提交 Bug 和需求的平台；登录信息只保存在当前部署机的 .local 中。</p></div><button className="primary compact" onClick={openNew}>添加反馈源</button></div>
    {notice && <button className="notice-strip" onClick={() => setNotice('')}>{notice}</button>}
    {providers.length === 0 ? <div className="empty-state"><h2>还没有工单反馈来源</h2><p>连接 Redmine 或 TAPD 后，CodeFixer 才能收取真实 Bug。</p><button className="primary compact" onClick={openNew}>连接反馈源</button></div> : <div className="provider-grid refined-provider-grid">{providers.map(provider => {
