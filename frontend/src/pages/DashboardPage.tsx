...
const stageLabels:Record<string,string>={prepare:'准备',scope_discovery:'范围',discovery:'定位',assess:'评估',workspace_prepare:'工作区',no_change_verify:'确认',repair:'修复',verify:'验证',review:'复核',pre_delivery_check:'检查',freeze_change:'冻结',deliver:'交付'}
...
function rail(run:TaskRun):StageItem[]{const stages=(run.stages??[]) as StageRun[];const grouped=new Map<string,StageRun>();for(const item of stages)grouped.set(item.stage_id,item);const order=['prepare','scope_discovery','discovery','assess','repair','verify','review','deliver'];return order.map(id=>{const item=grouped.get(id);return{label:stageLabels[id]??id,meta:item?duration(item):'',state:item?state(item.status):'queued'}})}
