...
const stageLabels:Record<string,string>={prepare:'准备',discovery:'分析',no_change_verify:'确认',repair:'修复',verify:'验证',review:'复核',pre_delivery_check:'检查',freeze_change:'冻结',deliver:'交付'}
...
function rail(run:TaskRun):StageItem[]{const stages=(run.stages??[]) as StageRun[];const grouped=new Map<string,StageRun>();for(const item of stages)grouped.set(item.stage_id,item);const order=['prepare','discovery','repair','verify','review','deliver'];return order.map(id=>{const item=grouped.get(id);return{label:stageLabels[id]??id,meta:item?duration(item):'',state:item?state(item.status):'queued'}})}
