    ...
    {projects.length === 0 ? <div className="empty-state project-empty"><span className="empty-symbol"><svg viewBox="0 0 36 36"><path d="M8 9h20v18H8z"/><path d="M13 5v8M23 5v8M5 15h6M25 15h6M13 22h10"/></svg></span><h2>建立第一条修复通道</h2><p>从一个工单反馈来源开始，再告诉 CodeFixer 去哪里理解问题、修改哪份工程，以及最终如何交付。</p><button className="primary compact" onClick={() => openEditor()}>开始配置</button></div> :
      <div className="project-grid">{projects.map(project => {
        ...
        return <article className="project-card" data-health={project.enabled === false ? 'inactive' : pf?.ready ? 'ready' : 'failed'} key={project.id}>
          ...
          <div className="project-card-actions"><button className={`pipeline-state-button ${project.enabled === false ? 'enable' : 'disable'}`} disabled={busy === `enabled:${project.id}`} onClick={() => void toggleEnabled(project)}><svg viewBox="0 0 16 16" aria-hidden="true"><path d="M8 2v5M4.3 4.2a5 5 0 1 0 7.4 0"/></svg>{busy === `enabled:${project.id}` ? '切换中…' : project.enabled === false ? '启用' : '停用'}</button><button className="ghost framed" onClick={() => openEditor(project)}><svg viewBox="0 0 16 16" aria-hidden="true"><path d="m3 11.8-.5 2 2-.5 7.8-7.8-1.5-1.5zM9.8 5l1.5 1.5M3 11.8l1.5 1.5"/></svg>编辑</button></div>
      ...
      <div className="workbench-layout">
        ...
        <div className="workbench-body">
          ...
          {step === 2 && <div className="step-panel"><div className="step-intro"><span>02</span><div><h3>为问题定位提供足够线索</h3><p>这是只读的辅助资料，可以是日志、分析工程或代码仓库；它不等于最终会被修改的工程。</p></div></div>
            ...
            <label className="localization-prompt-field"><span>定位补充说明 <em>可选</em></span><textarea value={editor.localizationPrompt ?? ''} onChange={e => setEditor({...editor,localizationPrompt:e.target.value})} maxLength={8000} placeholder="补充本项目特有的目录、命名或排查习惯；留空时仅使用平台规则与工单内容。"/><small>这段说明会追加给定位 Agent，用于领域导航；不能覆盖只读与安全边界。</small></label>
