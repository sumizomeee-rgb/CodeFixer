...
const defaultMr=():GitLabMrActionConfig=>({id:'main-gitlab-mr',type:'gitlabMr',connectionRef:'',projectPath:'',materializationRepositoryRef:'git-materialization',gitExecutableRef:'git-cli',targetBranches:['main'],pathMappings:[{from:'.',to:'.'}],titleTemplate:'[CodeFixer] {task_id}',descriptionTemplate:'Automated repair from CodeFixer run {run_id}.'})

export function FinalActionsEditor({actions,pathIds,connectionIds,onChange}:{actions:FinalActionConfig[];pathIds:string[];connectionIds:string[];onChange:(actions:FinalActionConfig[])=>void}){
