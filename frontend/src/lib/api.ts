...
export const api={
  ...
  projects:()=>request<{items:ProjectConfig[];etag:string;health:Record<string,PreflightResult>}>('/api/projects'),
  createProject:(project:ProjectConfig,etag:string)=>request<{project:ProjectConfig;etag:string;health:PreflightResult}>('/api/projects',{method:'POST',headers:{'If-Match':etag},body:JSON.stringify(project)}),
  updateProject:(projectId:string,project:ProjectConfig,etag:string)=>request<{project:ProjectConfig;etag:string;health:PreflightResult}>(`/api/projects/${encodeURIComponent(projectId)}`,{method:'PUT',headers:{'If-Match':etag},body:JSON.stringify(project)}),
  setProjectEnabled:(projectId:string,enabled:boolean,etag:string)=>request<{project:ProjectConfig;etag:string;health:PreflightResult|null}>(`/api/projects/${encodeURIComponent(projectId)}/enabled`,{method:'PUT',headers:{'If-Match':etag},body:JSON.stringify({enabled})}),
