...
export default function App(){
 const[dark,setDark]=useState(false);const[page,setPage]=useState<PageId>('dashboard');const[mode,setMode]=useState<ExecutionMode>('automatic');const[etag,setEtag]=useState('');const[ready,setReady]=useState<boolean|null>(null);const[confirmMode,setConfirmMode]=useState(false);const[toast,setToast]=useState('')
 useEffect(()=>{document.documentElement.dataset.theme=dark?'dark':'light'},[dark]);useEffect(()=>{api.settings().then(r=>{setMode(r.config.execution.mode);setEtag(r.etag)}).catch(()=>{});api.readiness().then(r=>setReady(r.ready)).catch(()=>setReady(false))},[])
 const toggleMode=async()=>{if(!etag){setToast('配置尚未加载，稍后重试');return}const next:ExecutionMode=mode==='automatic'?'awaitingStart':'automatic';try{const r=await api.setExecutionMode(next,etag);setMode(r.mode);setEtag(r.etag);setConfirmMode(false);setToast(next==='automatic'?'已切换为全自动':'已切换为待我开始')}catch(e){setToast(e instanceof Error?e.message:'模式切换失败')}}
 ...
 <main>{page==='dashboard'?<DashboardPage/>:page==='tasks'?<TasksPage/>:page==='projects'?<ProjectsPage/>:page==='settings'?<SettingsPage onModeChanged={(m,e)=>{setMode(m);setEtag(e)}}/>:<section className="page-stack"><div className="page-heading"><div><span className="eyebrow">PROVIDERS</span><h1>工单来源</h1><p>Provider Adapter 将持续收录 Bug，并用稳定游标与 Snapshot 保证幂等。</p></div></div></section>}</main>
 {confirmMode&&<div className="modal-backdrop"><div className="confirm-modal" role="dialog" aria-modal="true"><span className="signal-kicker">EXECUTION MODE</span><h2>{mode==='automatic'?'切换到「待我开始」？':'切换到「全自动」？'}</h2><p>{mode==='automatic'?'新收录的 Bug 将等待你点击一次开始；已经授权的任务继续执行。':'已收录且仍 eligible 的待开始任务会进入队列，所有确定性门禁仍然生效。'}</p><div className="modal-actions"><button className="ghost" onClick={()=>setConfirmMode(false)}>取消</button><button className="primary compact" onClick={()=>void toggleMode()}>确认切换</button></div></div></div>}{toast&&<button className="toast" onClick={()=>setToast('')}>{toast}</button>}</div>
}
