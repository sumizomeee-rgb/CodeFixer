...
class GitLabMrFinalAction:
    ...
    def __init__(self, *, action_id: str, action_version: int, delivery: GitLabMrDelivery, config: dict[str, object], target_branches: tuple[str, ...], work_root: Path, title_template: str = "[CodeFixer] {task_id}", description_template: str = "Automated repair from CodeFixer run {run_id}.") -> None:
    ...
    def execute(self, context: FrozenDeliveryContext) -> FinalActionResult:
        results = self.delivery.deliver(task_id=context.task_id, run_id=context.run_id, action_id=self.action_id, action_version=self.action_version, config=self.config, frozen_patch=context.patch_path, frozen_manifest=context.manifest_path, path_mappings=tuple((str(item.get("from", ".")), str(item.get("to", "."))) for item in self.config.get("pathMappings", []) if isinstance(item, dict)), base_revision=context.base_revision, target_branches=self.target_branches, work_root=self.work_root / context.run_id / self.action_id, title=self.title_template.format(task_id=context.task_id, run_id=context.run_id), description=self.description_template.format(task_id=context.task_id, run_id=context.run_id))
