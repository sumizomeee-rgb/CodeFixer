...
class TaskStore:
    ...
    def ingest(
        ...
        with self.connection:
            row = self.connection.execute(
                "SELECT id,current_snapshot_id FROM ticket_records "
                "WHERE provider_instance_id=? AND external_ticket_id=?",
                (ticket.provider_instance_id, ticket.external_ticket_id),
            ).fetchone()
            if row is None:
                cursor = self.connection.execute(
                    """INSERT INTO ticket_records(
                           provider_instance_id,external_ticket_id,title,eligible,external_version
                       ) VALUES (?,?,?,?,?)""",
                    (
                        ticket.provider_instance_id,
                        ticket.external_ticket_id,
                        ticket.title,
                        int(ticket.eligible),
                        ticket.external_version,
                    ),
                )
                ...
                self.connection.execute(
                    """UPDATE ticket_records
                       SET title=?,eligible=?,external_version=?,updated_at=CURRENT_TIMESTAMP
                       WHERE id=?""",
            ...
            if snap is None:
                cursor = self.connection.execute(
                    """INSERT INTO ticket_snapshots(
                           ticket_record_id,content_hash,title,payload_json,external_version
                       ) VALUES (?,?,?,?,?)""",
                    (
                        record_id,
                        content_hash,
                        ticket.title,
                        canonical_json(ticket.payload),
                        ticket.external_version,
                    ),
                )
                if cursor.lastrowid is None:
            ...
            self.connection.execute(
                "UPDATE ticket_records SET current_snapshot_id=? WHERE id=?",
                (snapshot_id, record_id),
            )
            task = self.connection.execute(
                "SELECT * FROM tasks WHERE ticket_record_id=?", (record_id,)
            ).fetchone()
            ...
            if task is None:
                ...
                self.connection.execute(
                    """INSERT INTO tasks(id,ticket_record_id,project_id,status,failure_json)
                       VALUES (?,?,?,?,?)""",
                    (
                        task_id,
                        record_id,
                        binding,
                        status,
                        canonical_json(failure) if failure is not None else None,
                    ),
                )
                ...
                if (is_new_snapshot or route_recovered) and task["status"] not in {
                    "queued",
                    "running",
                    "cancel_requested",
                }:
                    if failure is not None:
                        self.connection.execute(
                            """UPDATE tasks SET project_id=?,status='failed',failure_json=?,
                               updated_at=CURRENT_TIMESTAMP WHERE id=?""",
                            (binding, canonical_json(failure), task_id),
                        )
                    elif not ticket.eligible:
                        self.connection.execute(
                            """UPDATE tasks SET project_id=?,status='canceled',failure_json=NULL,
                               updated_at=CURRENT_TIMESTAMP WHERE id=?""",
                            (binding, task_id),
                        )
                    else:
                        ...
                        self.connection.execute(
                            """UPDATE tasks SET project_id=?,status=?,failure_json=NULL,
                               updated_at=CURRENT_TIMESTAMP WHERE id=?""",
                            (binding, next_status, task_id),
                        )
    ...
    def _create_run(self, task_id: str, execution_mode: str) -> str:
        ...
        self.connection.execute(
            """UPDATE tasks SET current_run_id=?,status=?,failure_json=NULL,
               updated_at=CURRENT_TIMESTAMP WHERE id=?""",
    ...
    def cancel_task(self, task_id: str) -> dict[str, Any]:
        ...
        with self.connection:
            if row["status"] in {"awaiting_start", "queued"}:
                self.connection.execute(
                    "UPDATE tasks SET status='canceled',updated_at=CURRENT_TIMESTAMP WHERE id=?",
                    (task_id,),
                )
                if row["current_run_id"]:
                    self.connection.execute(
                        "UPDATE task_runs SET status='canceled',finished_at=? WHERE id=?",
                        (_now(), row["current_run_id"]),
                    )
                self._event(task_id, "task_canceled", {})
            elif row["status"] == "running":
                self.connection.execute(
                    "UPDATE tasks SET status='cancel_requested',updated_at=CURRENT_TIMESTAMP WHERE id=?",
                    (task_id,),
                )
    ...
    def get_cursor(self, provider_instance_id: str) -> str | None:
        row = self.connection.execute(
            "SELECT cursor_value FROM provider_cursors WHERE provider_instance_id=?",
    ...
    def set_cursor(self, provider_instance_id: str, cursor: str | None) -> None:
        ...
        with self.connection:
            self.connection.execute(
                """INSERT INTO provider_cursors(provider_instance_id,cursor_value)
                   VALUES (?,?) ON CONFLICT(provider_instance_id) DO UPDATE SET
                   cursor_value=excluded.cursor_value,updated_at=CURRENT_TIMESTAMP""",
                (provider_instance_id, cursor),
            )

    def list_tasks(self, status: str | None = None) -> list[dict[str, Any]]:
        sql = (
            "SELECT t.*,tr.provider_instance_id,tr.external_ticket_id,tr.title "
    ...
    def get_task(self, task_id: str) -> dict[str, Any]:
        ...
        events = self.connection.execute(
            """SELECT id,event_type,payload_json,created_at FROM task_events
               WHERE json_extract(payload_json,'$.taskId')=? ORDER BY id""",
            (task_id,),
        ).fetchall()
        result["events"] = [
