...
def _config(tmp_path: Path) -> tuple[Path, dict[str, object]]:
    config_path=tmp_path/"config/defaults/codefixer.json";config_path.parent.mkdir(parents=True);project={"id":"demo","name":"Demo","finalActions":[{"id":"primary-patch","type":"patch","outputDirectory":str((tmp_path/"data/patches").resolve()),"filenameTemplate":"{task_id}-{run_id}.patch"}]}
...
def test_frozen_patch_delivery_resumes_after_restart_without_duplicate_output(tmp_path: Path):
    ...
    with connect_database(loaded.data_root/"codefixer.db") as connection:
        ...
        patch_output=loaded.data_root/"patches"/f"{task['id']}-{run_id}.patch";patch_output.parent.mkdir(parents=True);patch_output.write_bytes(patch_text.encode("utf-8"))
