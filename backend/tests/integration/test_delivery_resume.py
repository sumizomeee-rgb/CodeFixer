...
def test_frozen_patch_delivery_resumes_after_restart_without_duplicate_output(tmp_path: Path):
    ...
    with connect_database(loaded.data_root/"codefixer.db") as connection:
        ...
        run_root=loaded.data_root/"tasks"/task["id"]/"runs"/run_id;freeze_root=run_root/"freeze-change";freeze_root.mkdir(parents=True);(freeze_root/"change.patch").write_text(patch_text,encoding="utf-8")
        ...
        patch_output=loaded.data_root/"patches"/f"{task['id']}-{run_id}.patch";patch_output.parent.mkdir(parents=True);patch_output.write_text(patch_text,encoding="utf-8")
