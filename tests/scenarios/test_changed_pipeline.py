...
class DiscoveryAgent:
    ...
    def run(self, request):
        scope_path = next(path for label, path in _entry_paths(request.entry_file) if label == "Untrusted scope investigation notes")
        assert scope_path.name == "scope-discovery.md" and "untrusted navigation hints" in scope_path.read_text(encoding="utf-8")
...
class ScopeDiscoveryAgent:
    runtime_name = "fake"
    def __init__(self, revision: str): self.revision = revision
    def run(self, request):
        return AgentRunResult(status="succeeded", exit_code=0, session_id="s1", structured_output={"schema_version":1,"outcome":"candidates","source":{"id":"project-source","revision":self.revision},"summary":"Likely value implementation.","candidate_scope":["src/app.py"],"search_entry_points":["Search get_value in src/app.py"],"limitations":[],"failure":None})
...
def test_normal_changed_patch_journey(tmp_path: Path):
    ...
    with connect_database(db) as connection:
        ...
        scope_root=data/"tasks"/task["id"]/"runs"/run_id/"stages/scope_discovery";discovery_root=data/"tasks"/task["id"]/"runs"/run_id/"stages/discovery";scope_doc=scope_root/"scope-discovery.md";discovery_entry=(discovery_root/"entry.md").read_text(encoding="utf-8");assert scope_doc.is_file() and str(scope_doc.resolve()) in discovery_entry;assert "Likely value implementation" not in discovery_entry;assert json.loads((scope_root/"agent-runtime.json").read_text())["session_id"]=="s1";assert json.loads((discovery_root/"agent-runtime.json").read_text())["session_id"]=="d1"
