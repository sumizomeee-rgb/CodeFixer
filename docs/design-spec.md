...
- 名称、稳定 ID、启用状态和配置版本。
- 唯一工单入口：一个 `providerRef`、接收版本范围与带优先级的工单匹配规则。
...
工单入口的“接收版本”是路由范围，不是交付日志配置。选择反馈源后，平台从 TAPD 版本目录或 Redmine 可见修复版本生成选项，用户只选择业务版本，不填写字段 key。默认“全部版本”，包含尚未填写合入/修复版本的工单；切换“指定版本”后支持多选，未填写版本或未命中所选版本的工单不收录。版本范围与高级字段条件取交集。配置保存稳定版本 ID 与显示名称；Redmine 优先按 `fixed_version.id` 匹配，TAPD 因 Bug 的 `version_fix` 返回名称而使用精确名称匹配。
...
1. **工单入口**：选择或原地新增 TAPD/Redmine 接入；“接收版本”默认全部，也可从反馈源版本目录多选指定版本，再设置其他匹配范围和优先级。
